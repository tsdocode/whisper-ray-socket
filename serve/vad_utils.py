...
def read_audio(path: str, sampling_rate: int = 16000):
    ...
    if len(sox_backends.intersection(audio_backends)) > 0:
        ...
        wav, sr = torchaudio.sox_effects.apply_effects_tensor(
            wav, effects=effects, sample_rate=sr
        )
...
def save_audio(path: str, tensor: torch.Tensor, sampling_rate: int = 16000):
    torchaudio.save(
        path, tensor.unsqueeze(0), sampling_rate, bits_per_sample=16
    )
...
def get_speech_timestamps(
    ...
    speech_probs = []
    for current_start_sample in range(
        0, audio_length_samples, window_size_samples
    ):
        chunk = audio[
    ...
    if (
        current_speech
        and (audio_length_samples - current_speech["start"])
        > min_speech_samples
    ):
